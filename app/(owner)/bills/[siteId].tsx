...
export default function GSTBilling() {
  ...
  const fetchBills = async () => {
    try {
      const response = await fetch(`${API_BASE_URL}${BILLS_ENDPOINTS.GET_ALL(siteId!)}`);
  ...
  const uploadImage = async (): Promise<string | null> => {
    ...
    try {
      setUploading(true);
      const fileName = `bills/${siteId}-${Date.now()}.jpg`;
  ...
  const scanBill = async () => {
    ...
    try {
      setScanning(true);
      const response = await fetch(`${API_BASE_URL}${BILLS_ENDPOINTS.OCR}`, {
        method: 'POST',
      ...
      if (data.success && data.ocrData) {
        ...
        if (ocr.amount) setFormData({ ...formData, amount: ocr.amount.toString() });
        if (ocr.vendorName) setFormData({ ...formData, vendorName: ocr.vendorName });
        if (ocr.gstNumber) setFormData({ ...formData, gstNumber: ocr.gstNumber });
        if (ocr.billNumber) setFormData({ ...formData, billNumber: ocr.billNumber });
        Alert.alert('OCR Complete', 'Bill scanned. Please review and fill in missing details.');
      } else {
        Alert.alert('Info', 'OCR processing completed. Please enter bill details manually.');
      }
    } catch (error) {
      console.error('OCR error:', error);
      Alert.alert('Error', 'Failed to scan bill. Please enter details manually.');
    } finally {
      setScanning(false);
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        fetchBills();
        Alert.alert('Success', editingBill ? 'Bill updated' : 'Bill added');
      } else {
      ...
      <LinearGradient colors={['#8B5CF6', '#7C3AED']} style={styles.header}>
        <View style={styles.headerContent}>
          ...
          <View style={styles.headerText}>
            ...
            <Text style={styles.headerSubtitle}>Track Expenses & Bills</Text>
          </View>
        ...
        {bills.length === 0 ? (
          ...
          bills.map((bill) => (
            <View key={bill.id} style={styles.billCard}>
              ...
              {(bill.vendorName || bill.gstNumber || bill.billNumber) && (
                <View style={styles.billDetails}>
                  {bill.vendorName && (
                    <View style={styles.detailRow}>
                      <Ionicons name="business" size={16} color="#6B7280" />
                      <Text style={styles.detailText}>{bill.vendorName}</Text>
                    </View>
                  )}
                  {bill.gstNumber && (
                    <View style={styles.detailRow}>
                      <Ionicons name="document-text" size={16} color="#6B7280" />
                      <Text style={styles.detailText}>GST: {bill.gstNumber}</Text>
                    </View>
                  )}
                  {bill.billNumber && (
                    <View style={styles.detailRow}>
                      <Ionicons name="hash" size={16} color="#6B7280" />
                      <Text style={styles.detailText}>Bill #: {bill.billNumber}</Text>
                    </View>
                  )}
                </View>
              )}
            ...
            <ScrollView style={styles.modalBody}>
                    ...
                    >
                      {scanning ? (
                        ...
                        <>
                          ...
                          <Text style={[styles.imageButtonText, styles.scanButtonText]}>Scan</Text>
                        </>
                      )}
                    </TouchableOpacity>
                  )}
              ...
              <View style={styles.inputGroup}>
                ...
                <TextInput
                  ...
                  placeholder="e.g., Material purchase, Equipment rental"
                  multiline
                  numberOfLines={3}
                />
              </View>
...
const styles = StyleSheet.create({
  ...
  billReason: { fontSize: 16, color: '#374151' },
  billThumbnail: { width: 80, height: 80, borderRadius: 8, marginLeft: 12 },
  billDetails: { marginBottom: 12, gap: 8 },
  ...
  scanButton: { backgroundColor: '#8B5CF6' },
  imageButtonText: { fontSize: 14, fontWeight: '600', color: '#8B5CF6' },
  scanButtonText: { color: '#fff' },
